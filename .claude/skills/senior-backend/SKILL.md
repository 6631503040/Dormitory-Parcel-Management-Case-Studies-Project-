...
> NOTE: This SKILL.md references bundled helpers (`scripts/`, `references/`, `profiles/`,
> `agents/`) that were NOT included when the skill was pulled in. The workflow guidance
> below is usable as-is; the `python scripts/*.py` commands and `references/*.md` files
> will not exist until those assets are added.
